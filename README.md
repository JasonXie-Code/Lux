...
读取手机光线传感器，在屏幕中央大字显示当前照度（lux），数值保留两位小数；界面按所选间隔刷新显示，可在 **200ms、500ms、1s、2s** 四档中切换（默认 **500ms**），选择会持久保存。
...
- 照度显示为两位小数；刷新间隔由界面上的刷新率控制切换（200ms / 500ms / 1s / 2s，默认 500ms），并写入本地偏好。
